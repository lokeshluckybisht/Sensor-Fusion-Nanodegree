...
std::unordered_set<int> RansacPlane(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	while(maxIterations--)
	{
		...
		const float j{static_cast<float>(((z2-z1)*(x3-x1)) - ((z2-z1)*(y3-y1)))};
		...
		const float eucledianDist{std::sqrt((a*a)+(b*b)+(c*c))};

		for(int index{0} ; index < cloud->points.size(); index++)
		{
			const float plane{std::fabs((a*cloud->points.at(index).x)+(b*cloud->points.at(index).y)+(c*cloud->points.at(index).z)+ d)};

			if((plane/eucledianDist)<= distanceTol){inliers.insert(index);}
		}
		if(inliers.size()>inliersResult.size())
			{
				inliersResult = inliers;
