...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* pointProcessor, pcl::PointCloud<pcl::PointXYZI>::Ptr& inputCloud)
{
	...
	// ----------------------------------------------------
	inputCloud = pointProcessor->FilterCloud(inputCloud, 0.25 , Eigen::Vector4f (-25, -6.3, -3, 1), Eigen::Vector4f ( 30, 7.3, 3, 1));
	
    std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = pointProcessor->SegmentPlane(inputCloud, 100, 0.25);
	std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessor->Clustering(segmentCloud.first, 0.4, 15, 1000);
